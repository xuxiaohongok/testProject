...
public class IPSeeker {   
    ...
    private static final String IP_FILE = IPSeeker.class.getResource("/qqwry.dat").toString().replace("%20", " ").substring(5);
...
//      buf = new byte[100];//不需要了，这里初始化不好!   
        ...
        for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {   
            ...
            if (temp != -1) {   
                ...
                if (loc.country.indexOf(s) != -1 || loc.area.indexOf(s) != -1) {   
                    ...
                    // 得到起始IP   
                    readIP(offset - 4, b4);   
                    entry.beginIp = IpStringFormat.getIpStringFromBytes(b4);   
                    // 得到结束IP   
                    readIP(temp, b4);   
                    entry.endIp = IpStringFormat.getIpStringFromBytes(b4);   
                    // 添加该记录   
                    ret.add(entry);   
                }   
            }   
    ...
    public String getCountry(byte[] ip) {   
        ...
        if (ipFile == null)   
            return "错误的IP数据库文件";   
        // 保存ip，转换ip字节数组为字符串形式   
        String ipStr = IpStringFormat.getIpStringFromBytes(ip);   
    ...
    public String getCountry(String ip) {   
        return getCountry(IpStringFormat.getIpByteArrayFromString(ip));   
    }   
    ...
    public String getArea(String ip) {   
        return getArea(IpStringFormat.getIpByteArrayFromString(ip));   
    }   
    ...
    private String readString(long offset) {   
        try {   
            ...
            /////////   
            if (i != 0)   
                return IpStringFormat.getString(buf, 0, i, "GBK");   
        } catch (IOException e) {   
            System.out.println(e.getMessage());   
        }   
        return "";   
...
//    	for (int i = 0; i < 1; i++) {
//    		System.out.println(IPSeeker.getInstance().getAddress("223.81.207.72"));
//		}
		
         long startTime = System.currentTimeMillis();
         for(int i=0;i<10000;i++) {
         	String lo = IPSeeker.getInstance().getAddress("58.61.220.22");
         	System.out.println(lo);
         }
         
         long endTime = System.currentTimeMillis();
         System.out.println(endTime - startTime);
		//System.out.println(IPSeeker.class.getResource("/cache/qqwry.dat").toString().substring(5));
	}
}
